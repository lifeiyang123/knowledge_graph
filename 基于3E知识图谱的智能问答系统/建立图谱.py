...
import py2neo
#graph直接写账号密码会不安全
g=Graph('http://localhost:7474/browser/',auth=('neo4j','5201314lfy'),name='neo4j')
#资源与环境
...
# with open('D:/知识图谱智能问答/NLP_process-main1/NLP_process-main/基于3E知识图谱的智能问答系统/data/4-联系/3-环境/4-环境具体分类.csv','r',encoding='utf-8') as f:
#     #数据集除了第一行代表属性外，第一列为实体1，第二列为实体2，第三列是两者英文关系，第四列为两者中文关系
#     reader=csv.reader(f)
#     for item in reader:
#         #第一行的标签不是咱们需要的内容，line_num表示文件的第几行
...
# with open('D:/知识图谱智能问答/NLP_process-main1/NLP_process-main/基于3E知识图谱的智能问答系统/data/4-联系/3-环境/5-环境分类-环境具体分类.csv', 'r', encoding='utf-8') as f:
#     #数据集除了第一行代表属性外，第一列为实体1，第二列为实体2，第三列是两者英文关系，第四列为两者中文关系
#     reader=csv.reader(f)
#     for item in reader:
#         #第一行的标签不是咱们需要的内容，line_num表示文件的第几行
...
#             continue
#         # print("当前行数：", reader.line_num, "当前内容", item)
#         findnode = matcher.match('环境分类_4_3_2', id=item[0]).first()
#         endnode = matcher.match('环境具体分类_4_3_4', id=item[1]).first()
#         # print(findnode,endnode)
#         relationships = Relationship(findnode, '.分为', endnode)
#         g.merge(relationships, "", "id")

# with open('D:/知识图谱智能问答/NLP_process-main1/NLP_process-main/基于3E知识图谱的智能问答系统/data/4-联系/3-一次能源.csv','r',encoding='utf-8') as f:
#     #数据集除了第一行代表属性外，第一列为实体1，第二列为实体2，第三列是两者英文关系，第四列为两者中文关系
#     reader=csv.reader(f)
#     for item in reader:
#         #第一行的标签不是咱们需要的内容，line_num表示文件的第几行
...
#             continue
#         # print("当前行数：",reader.line_num,"当前内容",item)
#         test_node_1=Node("一次能源_4_3",id=item[0],name=item[1])
#         g.merge(test_node_1, "一次能源_4_3", "id")
