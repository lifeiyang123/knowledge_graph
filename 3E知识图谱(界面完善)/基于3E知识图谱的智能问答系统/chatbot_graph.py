...
import openai
from relation_search import *
from definition_search import *
'''问答类'''
class ChatBotGraph:
    def clear(self):
        os.system('cls')
...
class SearchPage:
    def __init__(self,master):
        ...
        photo = ImageTk.PhotoImage(file=r'.\背景图片\1_修改.jpg')
        label = tk.Label(self.root, image=photo, width=650, height=650)
        label.place(x=0)
        ...
        def huoqu():
            ...
            #指定查询工具
            if question[0]==' ':
                openai.api_key = "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"
                question=question.replace(' ','')
                prompt=question+',please answer in Chinese'
            ...
            #以下是爬取内容
                    ...
                    question=lfy
                    openai.api_key = "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"
                    prompt = question + ',Please answer in Chinese'
                    completions = openai.Completion.create(
                        # engine="text-davinci-002",
                        engine="text-davinci-002",
        ...
        lfy = ''
        b = tk.Label(self.root, text='3E Knowledge Graph', font=('宋体', 40), fg='mediumblue')
        b.place(x=110, y=140)

        c = tk.Button(self.root, text='查询', command=huoqu, width=4, height=1, bg='skyblue', fg='red')
        c.place(x=90, y=260)
        # c.pack()
        a = tk.Entry(self.root, textvariable=shuru, width=60, fg='red')  # .grid(row=1,column=2)
        a.place(x=130, y=260)

        wenzi = ['北京温度', '耕地的面积', '能源的定义', '石油与能源的关系']
        suiji = random.sample(wenzi, 3)

        def xswz0():
            a.delete(0, 'end')
            a.insert(INSERT, suiji[0])
            huoqu()

        def xswz1():
            a.delete(0, 'end')
            a.insert(INSERT, suiji[1])
            huoqu()

        def xswz2():
            a.delete(0, 'end')
            a.insert(INSERT, suiji[2])
            huoqu()

        d = tk.Button(self.root, text=suiji[0], font=('宋体', 10), fg='mediumblue', command=xswz0,width=15)
        d.place(x=130, y=300)

        f = tk.Button(self.root, text=suiji[1], font=('宋体', 10), fg='mediumblue', command=xswz1,width=15)
        f.place(x=260, y=300)

        h = tk.Button(self.root, text=suiji[2], font=('宋体', 10), fg='mediumblue', command=xswz2,width=15)
        h.place(x=390, y=300)

        e = tk.Label(self.root, text='回答', width=4, bg='skyblue')
        e.place(x=90, y=370)

        text = Text(self.root, width=60, height=10)
        text.place(x=130, y=370)

        g = tk.Button(self.root, text='返回登录页', command=fanhui, width=8, height=1, bg='skyblue', fg='red')
        g.place(x=580, y=500)

        b = tk.Label(self.root, text='@版权所有:中国石油大学(北京)3E知识图谱团队')
        b.place(x=390, y=625)
